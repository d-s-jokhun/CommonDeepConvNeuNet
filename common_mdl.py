...
# MasterPath = os.path.abspath('//deptnas.nus.edu.sg/BIE/MBELab/jokhun/Pro 1/Cellular biosensor')
# MasterPath = os.path.abspath('/hpctmp/biejds/Pro 1/Cellular biosensor')
# MasterPath = os.path.abspath('/MBELab/jokhun/Pro 1/Cellular biosensor')

model_name = 'DICFea1Scra1412'

ImgPath = join (MasterPath,'6. Data/Projected_Fea1')
...
# sample_id = np.random.choice([i for i in range(len(Ch1_img_paths))], size=8, replace = False)
...
numOfCrops_per_dim = 2
def load_img (path, numOfCrops_per_dim=numOfCrops_per_dim):
    raw = tifffile.imread(path)

    img_size = raw.shape[0]
    extra_pix = img_size % numOfCrops_per_dim
    extra_pix_0 = extra_pix//2
    extra_pix_1 = extra_pix - extra_pix_0
    raw = raw [extra_pix_0:img_size-extra_pix_1,extra_pix_0:img_size-extra_pix_1]

    crops=np.array([img for crop_ls in (np.split(split_0,numOfCrops_per_dim,-1) for split_0 in np.split(raw,numOfCrops_per_dim,0)) for img in crop_ls])
    return crops
    # return cv2.resize(tifffile.imread(path), (1412,1412), interpolation = cv2.INTER_AREA)
...
if __name__=='__main__':
    with mp.Pool() as pool:
        Ch1_img = np.concatenate(pool.map(load_img,Ch1_img_paths),axis=0) if len(Ch1_img_paths)>0 else []
        Ch2_img = np.concatenate(pool.map(load_img,Ch2_img_paths),axis=0) if len(Ch2_img_paths)>0 else []
        Ch3_img = np.concatenate(pool.map(load_img,Ch3_img_paths),axis=0) if len(Ch3_img_paths)>0 else []
        Ch4_img = np.concatenate(pool.map(load_img,Ch4_img_paths),axis=0) if len(Ch4_img_paths)>0 else []
        Ch5_img = np.concatenate(pool.map(load_img,Ch5_img_paths),axis=0) if len(Ch5_img_paths)>0 else []


#%% Repeating the paths to be consistent with image splitting

Ch1_img_paths = [path for path in Ch1_img_paths for rep in range(numOfCrops_per_dim*numOfCrops_per_dim)]
Ch2_img_paths = [path for path in Ch2_img_paths for rep in range(numOfCrops_per_dim*numOfCrops_per_dim)]
Ch3_img_paths = [path for path in Ch3_img_paths for rep in range(numOfCrops_per_dim*numOfCrops_per_dim)]
Ch4_img_paths = [path for path in Ch4_img_paths for rep in range(numOfCrops_per_dim*numOfCrops_per_dim)]
Ch5_img_paths = [path for path in Ch5_img_paths for rep in range(numOfCrops_per_dim*numOfCrops_per_dim)]


#%% Randomize the dataset

def randomizer (arr_ls, rng, randomizer_state):
    for arr in arr_ls:
        rng.__setstate__(randomizer_state)
        rng.shuffle(arr)
    return

rng = np.random.default_rng(seed=0)
randomizer_state = rng.__getstate__()

randomizer ([Ch1_img_paths, Ch2_img_paths, Ch3_img_paths, Ch4_img_paths, Ch5_img_paths], rng, randomizer_state)
randomizer ([Ch1_img, Ch2_img, Ch3_img, Ch4_img, Ch5_img], rng, randomizer_state)
...
def idx2img(batch_idx,batch_y,ImageSet):
    def fetch (batch_idx,ImageSet=ImageSet):
        ...
        return batch_x # np.concatenate((batch_x,batch_x,batch_x),axis=3)
    batch_x=tf.numpy_function(fetch, [batch_idx], tf.uint8)
    batch_x.set_shape((None,*ImageSet.shape[1:])) # ((None,*ImageSet.shape[1:3],3))
...
core_model = tf.keras.applications.Xception(input_shape=[*Final_img_shape[:2],3], include_top=False, weights="imagenet")
ref_core_model = "tf.keras.applications.Xception(input_shape=[*Final_img_shape[:2],3], include_top=False, weights=None)"
...
if AddTopAndBottomLayers:
    ...
    Dropout1 = tf.keras.layers.Dropout(0.25, name="Feature_Dropout")
    ...
    Dropout2 = tf.keras.layers.Dropout(0.25, name="Dropout2")
...
def train_model (model, Dataset_Tr, Dataset_Val, initial_epoch=0, final_epoch=5, Model_Path=None, class_names=None):
    ...
    EarlyStop_cb = tf.keras.callbacks.EarlyStopping(monitor='val_loss', \
        min_delta=0, patience=75, verbose=2, mode='auto',\
...
FineTune_Core_Onwards = 0  # 0 to set the entire model as trainable and None to copletely freeze training of the core model
